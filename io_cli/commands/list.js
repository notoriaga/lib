...
class GetCommand extends Command {
  ...
  help() {
    ...
    return {
      ...
      flags: {
        ...
        m: 'Only fetch file metadata'
      },
      vflags: {
        token: 'Library Token (default is currently active)',
        metadata: 'Only fetch file metadata'
      }
    };
  ...
  run(params, callback) {
    ...
    let meta = params.flags.hasOwnProperty('m') || params.vflags.hasOwnProperty('metadata');
    let endpoint = '/directories?path=' + encodeURIComponent(stdioPath)
    endpoint = meta ? endpoint + '&info=true' : endpoint
    ...
    return resource
      ...
      .stream(
        ...
        (err, result) => {
          ...
          return callback(null, result.toString())//JSON.stringify(result, null, 2))
        }
      )
